...
-- 物品主表：统一存储失物与招领信息
-- 状态由 status 字段统一管理，审核历史记录在 item_audits 表
CREATE TABLE items (
    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ...
    status VARCHAR(32) NOT NULL DEFAULT 'PENDING_REVIEW' COMMENT '状态：DRAFT(草稿)/PENDING_REVIEW(待审核)/PUBLISHED(已发布)/REJECTED(已驳回)/CLAIMED(已认领)/FOUND_BACK(已找回)/OFFLINE(已下线)',
    ...
    KEY idx_items_scene_stage (scene, stage),
    KEY idx_items_public_query (deleted_at, status, stage, created_at)
...
-- 物品审核历史表（只记录审核操作历史，状态由 items.status 统一管理）
CREATE TABLE item_audits (
    id BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    item_id BIGINT NOT NULL COMMENT '物品ID',
    audit_action VARCHAR(16) NOT NULL COMMENT '审核操作：APPROVED/REJECTED',
...
-- 认领审核历史表（只记录审核操作历史，状态由 claims.status 统一管理）
...
CREATE TABLE contact_unlock_logs (
    ...
    source VARCHAR(32) NOT NULL COMMENT '来源：DETAIL_PAGE/LIST_PAGE',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    ...
    KEY idx_contact_unlock_logs_created (created_at)
